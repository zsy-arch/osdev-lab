 ...
 *
 * 已经写好，不是 TODO：这个减法本身没什么可练的，但**什么时候该调用它**
 * 是本文件的一个考点——下面每一处填进描述符的地址都必须先过这个函数，
 * 漏掉任何一处的症状都是"设备看起来跑完了，但缓冲区里什么都没有"。 */
 ...
 *
 * 已经写好，不是 TODO：和 ide.c 把 inb/outb 直接给你一样——访问原语不是
 * 本 Lab 要练的东西，要练的是**用它们跟设备完成一次对话**。 */
...
/* TODO 1：实现 virtio_find_device——扫 8 个槽位，把找到的块设备基址写进
 * g_virtio_base。
 *
 * 为什么不直接 g_virtio_base = 0x10001000 了事：因为不指定 bus= 时 QEMU
 * **从高往低**分配槽位，只挂一块盘时它落在 0x10008000。硬编码第一个槽位的
 * 教程在这种命令行下会读到一个 DeviceID=0 的空槽，然后报"没有磁盘"，而盘
 * 其实就在隔壁。run-qemu.sh 现在显式把盘钉在 bus.0，但**驱动仍然要扫描**
 * ——环境约束和代码健壮性是两件事，不该互相替代：钉槽位是为了让行为确定，
 * 扫描是为了让驱动在没钉的时候也能工作。
 *
 * 步骤：
 *   1. for i in 0..VIRTIO_MMIO_SLOTS-1：
 *      先把 g_virtio_base 设成 VIRTIO_MMIO_BASE + i * VIRTIO_MMIO_STRIDE
 *      （mmio_read 是拿 g_virtio_base 当基址的，所以必须先设再读）。
 *   2. 读 MAGIC_VALUE，不等于 VIRTIO_MAGIC 就 continue：这个地址上根本不是
 *      virtio 传输通道。
 *   3. 读 DEVICE_ID，不等于 VIRTIO_DEVICE_ID_BLOCK 就 continue：是 virtio
 *      通道，但槽位空着（DeviceID 为 0）或挂的是别的设备。
 *   4. 读 VERSION。**不等于 2 要单独报错**，别跟"找不到设备"混在一起：
 *      版本 1 是 legacy 接口（QueuePFN 那一套），寄存器布局跟本文件上面
 *      那一串 #define 完全不同。这时要把该加的 QEMU 参数直接打出来：
 *        -global virtio-mmio.force-legacy=false
 *      错误信息要说清"下一步做什么"，而不只是"哪里不对"——这是本课程反复
 *      用的一条原则，而这个坑几乎每个人都会踩一次（QEMU 的 virtio-mmio
 *      默认 force-legacy=true）。
 *   5. 打一行 banner（tests/expect-riscv64.txt 会 grep 它）然后 return。
 *   6. 循环走完都没找到：把 g_virtio_base 清成 0，打印扫了哪些地址、提示
 *      检查 -drive/-device 参数，然后 panic。
 *
 * 顺便理解一下为什么"扫描"本身是常态：设备在哪个槽位是运行时才知道的
 * 事实，正常系统靠设备树（riscv/ARM）或 PCI 枚举（x86）得到这个信息。
 * 本 Lab 不实现设备树解析，用"遍历已知的地址窗口 + 看魔数"这个更朴素的
 * 办法，但"地址要发现、不要假设"这个态度是一样的。
 *
 * 注意 kprintf 没有宽度/精度支持，64 位地址要用 %lx。
 *
 * 提示：
...
/* TODO 2：实现 blk_init——走完 virtio 规范第 3.1.1 节的设备初始化序列。
 *
 * 为什么这个函数值得逐条照规范抄：设备内部是一个状态机，它靠 Status
 * 寄存器的写入来推进。**每一步都不能省、不能换顺序**；顺序错了设备会拒绝
 * 工作，而且通常不会报错，只是安静地什么都不做。这类"没有错误信息的失败"
 * 是写设备驱动最常见的挫败感来源——照着规范逐条对是唯一可靠的办法，也是
 * 这个 TODO 真正要教的习惯。
 *
 * 步骤：
 *   0. 先 virtio_find_device()（TODO 1）。
 *   1. 写 STATUS = 0 复位设备。**即使刚上电也要做**——不能假设设备处于
 *      初始状态（bootloader 可能已经用过它了；本课程 riscv 用 OpenSBI，
 *      它不碰 virtio，但这个假设不该依赖）。
 *   2. STATUS = ACKNOWLEDGE，然后 STATUS = ACKNOWLEDGE|DRIVER。
 *      这些位是**累积**的，用一个局部 status 变量 |= 上去再写。
 *      写成 mmio_write(STATUS, DRIVER) 会把 ACKNOWLEDGE 清掉，设备认为
 *      驱动退回了上一步。
 *   3. 特性协商：读一次 DEVICE_FEATURES（本 Lab 不用它的值，但保留这次读
 *      是为了让"协商"这个动作在代码里留下痕迹，调试时可以在这里打印看设备
 *      到底支持什么），然后写 DRIVER_FEATURES = 0——一个特性都不启用是
 *      合法的，所有可选特性都是可选的。
 *   4. status |= FEATURES_OK 写进去，**然后读回来确认这一位还在**。
 *      这是整个序列里唯一一个"写完要检查"的步骤，规范专门强调：设备如果
 *      不接受驱动选的特性组合，会把 FEATURES_OK 清掉。不读回就往下走，
 *      等于在一个设备已经拒绝了的配置上操作。
 *   5. QUEUE_SEL = 0（virtio-blk 只有一个队列；网卡那类设备会有多个）。
 *      顺手检查 QUEUE_READY 现在应该是 0——不是 0 说明第 1 步的复位没生效。
 *   6. 读 QUEUE_NUM_MAX：0 表示队列不存在；小于 VIRTQ_QUEUE_SIZE 表示设备
 *      装不下本驱动要的队列。两种都 panic（分开报，原因不同）。
 *      然后写 QUEUE_NUM = VIRTQ_QUEUE_SIZE。
 *   7. memset 清零 g_desc / g_avail / g_used，g_used_seen = 0。
 *      .bss 本来就是零，显式清一遍是为了让"队列从零状态开始"在代码里可见
 *      （也让将来支持重新初始化时行为正确）。avail.idx / used.idx 的初始值
 *      必须是 0，驱动记的 g_used_seen 才对得上。
 *   8. 用 virt_to_phys() 算出三块内存的**物理**地址，每块分低 32 位/高 32 位
 *      写进 DESC_LOW/HIGH、DRIVER_LOW/HIGH（avail 环）、DEVICE_LOW/HIGH
 *      （used 环）。拆成两个寄存器是因为 MMIO 寄存器都是 32 位而地址是 64 位。
 *   9. QUEUE_READY = 1。
 *  10. status |= DRIVER_OK 写进去。**这一位之后设备才会真正处理请求。**
 *  11. 打一行 banner（tests/expect-riscv64.txt 会 grep 它）。
 *
 * 第 8 步是本文件最容易错的地方：填进去的必须是物理地址。忘了调
 * virt_to_phys、直接把 g_desc 的虚拟地址写进寄存器，设备会去访问一个天文
 * 数字的物理地址——不报错，只是数据永远不出现。
 *
 * 提示：
 * void blk_init(void)
 * {
...
/* TODO 3：实现 blk_read——组装一条三段描述符链、提交、等完成、核对结果。
 * 整个 riscv 驱动的正主，也是本 Lab 唯一一处需要跟"设备视角"较真的地方。
 *
 * 为什么它比 ide.c 的 blk_read 长一倍：那边 CPU 亲自搬数据，一行 insw 就
 * 完事；这边要在内存里写好一张"我要什么"的表，让设备自己 DMA。多出来的
 * 篇幅全是在处理"两个处理器通过内存通信"带来的问题——地址视角、内存顺序、
 * 完成通知。这三件事在现代设备驱动（NVMe、网卡、GPU）里是通用的。
 *
 * 开头的守卫：
 *   - _Static_assert(BSIZE == 512, ...)：本 Lab 块大小恰好等于 virtio-blk
 *     的扇区单位，所以块号直接就是扇区号。真实文件系统用 4KiB 块时要乘 8。
 *   - g_virtio_base == 0 -> panic（blk_init() 没被调用）。
 *   - blkno >= FS_NBLOCKS -> kprintf + panic（上层算错了块号）。
 *
 * 步骤一：填请求头和状态字节
 *   g_req_header.type = VIRTIO_BLK_T_IN，reserved = 0，sector = blkno。
 *   g_req_status = 0xFF。**这个哨兵值很关键**：这样"设备根本没写这个字节"
 *   和"设备写了 OK(0)"就能区分开——初值是 0 的话，设备什么都没做也会被
 *   读成成功。检查硬件结果时的通用手法：哨兵值要落在合法结果集之外。
 *
 * 步骤二：串出 desc[0] -> desc[1] -> desc[2]
 *   本驱动一次只有一个在途请求，所以每次都从 0 号开始重用，不需要维护
 *   "哪些描述符空闲"的分配器（真实驱动需要，那是一个空闲链表）。
 *     desc[0]: addr = virt_to_phys(&g_req_header), len = sizeof(它),
 *              flags = NEXT（驱动->设备，**不设 WRITE**）, next = 1
 *     desc[1]: addr = virt_to_phys(buf), len = BSIZE,
 *              flags = NEXT | WRITE, next = 2
 *     desc[2]: addr = virt_to_phys(&g_req_status), len = 1,
 *              flags = WRITE（链尾，**不设 NEXT**）, next = 0
 *   VIRTQ_DESC_F_WRITE 的含义是"**设备**往这段内存写"——读磁盘时正是如此。
 *   这一位的命名视角是设备侧，设反了的症状是缓冲区里什么都没出现（设备
 *   认为这段是它要读的输入），很容易误判成"盘上是空的"。这是 virtio 里
 *   最容易设反的一位，也是本 TODO 的第一个考点。
 *   三个 addr 都必须过 virt_to_phys()：设备做 DMA 不经过 MMU。
 *
 * 步骤三：提交（三行代码，两个屏障）
 *   1. g_avail.ring[g_avail.idx % VIRTQ_QUEUE_SIZE] = 0;   放链头下标
 *   2. __asm__ volatile("fence rw, rw" ::: "memory");
 *   3. g_avail.idx++;
 *   4. 再来一次同样的 fence
 *   5. mmio_write(VIRTIO_MMIO_QUEUE_NOTIFY, 0);
 *
 *   第 2 步那个屏障是本 Lab 最"看不见效果"但最重要的一行：它保证上面对
 *   描述符和 avail.ring 的所有写入，在设备看到 avail.idx 的新值**之前**
 *   就已经可见。设备是另一个"处理器"，它通过内存看驱动的意图；CPU 和
 *   编译器都可以重排写入顺序，只要单线程语义不变——但 avail.idx 的递增
 *   是那个"现在归你了"的信号，必须最后到达。idx 先被看到、描述符内容后到，
 *   设备就会处理一条还没填好的请求，读到上一次的地址或者全零。
 *
 *   这类 bug 的可怕之处在于它是时序相关的：在 QEMU 这种顺序执行的模拟器
 *   上几乎永远不出现，在真实的乱序 CPU 上偶发。**屏障不是为了让当前环境
 *   跑通，是为了让代码在正确的模型下是对的**——去掉它你的测试照样全绿，
 *   这正是它值得被专门理解的原因。
 *
 *   riscv 的 `fence rw, rw` 表示"屏障之前的所有读写，都要排在屏障之后的
 *   所有读写之前"。x86 的强内存模型里对应的是 sfence/mfence。
 *
 * 步骤四：等完成
 *   while (g_used.idx == g_used_seen) { 计数超过 VIRTIO_POLL_LIMIT 就
 *   kprintf 细节 + panic；循环体里放一次 fence }。
 *   循环里那个 fence 有两个作用：每次循环都要重新从内存读 g_used.idx
 *   （设备改的是内存，编译器看不到有谁会改它，会把这个读提到循环外面变成
 *   死循环），以及提供多处理器可见性。这里用屏障而不是把 g_used 声明成
 *   volatile，是因为 volatile 只能阻止编译器优化、不提供任何可见性保证；
 *   fence 两件事都做到。把"设备会改的内存"和"需要屏障"绑在一起理解，比
 *   记住 volatile 的特例更可靠。
 *   跳出循环后再来一次 fence，才去读设备写进缓冲区和状态字节的内容——
 *   没有它 CPU 可能已经把旧的缓冲区内容预读进了寄存器/缓存。这是提交侧
 *   屏障的镜像，一进一出，两侧都要。
 *
 * 步骤五：核对
 *   - g_used.ring[g_used_seen % VIRTQ_QUEUE_SIZE].id 应该是 0（本驱动只
 *     提交过 0 号链）。不是就 panic：设备行为和假设不符要立刻发现，而不是
 *     默默用错数据。
 *   - g_used_seen++。
 *   - g_req_status == 0xFF -> panic（设备报告完成，但状态字节没被写过）。
 *   - g_req_status != VIRTIO_BLK_S_OK -> kprintf 状态码 + panic。
 *
 * 写完之后请注意一件事：**这个函数里没有任何一行在搬数据**。数据是设备
 * DMA 进 buf 的，CPU 一个字节都没碰过。对比 ide.c 那一行 insw，这就是
 * PIO 和 DMA 的全部区别。
 *
 * 另外，为什么记 g_used_seen 而不是"等 g_used.idx == 1"：idx 是**只增**的
 * 累积计数器，不会回到 0，第二次请求完成时它是 2 而不是 1。写死成等某个
 * 具体数值的代码只有第一次能工作。
 *
 * 提示：
