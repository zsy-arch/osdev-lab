 ...
 *
 * 为什么单独抽一个函数：下面六个系统调用都要做同一套 fd 合法性检查。收在
...
/* TODO 6：实现 sys_open(name) -> fd，失败返回 -1。
 *
 * 步骤：
 *   1. proc_current()，为 NULL 就 panic（系统调用只能来自用户进程）。
 *   2. fs_lookup(user_name) 拿 inum。**inum == 0 返回 -1，不要 panic。**
 *   3. fd_alloc(p)，< 0 就返回 -1（真实 Unix 在这里返回 EMFILE）。
 *   4. 填 p->ofile[fd]：type = FD_INODE、inum = 刚查到的、off = 0、
 *      pipe = NULL、writable = 0。返回 fd。
 *
 ...
 *
 * 第 4 步为什么每个字段都要显式赋值，而不是只改 type：TODO 1 里已经
 * 强调过，槽位复用时每次赋值都要自己负责完整初始化，不能依赖"上一次
 * close 清干净了"——那会把两处代码耦在一起。
 *
 * 提示：
...
/* TODO 7：实现 sys_read(fd, buf, len) -> 实际读到的字节数。
 *
 * 步骤：
 *   1. proc_current()，为 NULL 就 panic。
 *   2. fd_lookup()，为 NULL 返回 -1。
 *   3. 按 f->type 分派：
 *      - FD_CONSOLE：return console_read((char *)user_buf, len);
 *      - FD_INODE：got = fs_read(f->inum, f->off, user_buf, (uint32_t)len)；
 *        **f->off += got;** return (int64_t)got。
 *      - FD_PIPE：f->writable（拿错了 fd，从写端读）返回 -1；否则
 *        return (int64_t)pipe_read(f->pipe, (char *)user_buf, (uint32_t)len);
 *
 * FD_INODE 分支里 f->off += got 那一行就是"文件描述符携带状态"的全部
 * 实现：用户程序连续调用 read() 能读到文件的后续内容，靠的就是内核在
 * 这里替它记住了位置。漏掉这一行的症状很好认——每次 read 都从文件开头
 * 返回同样的内容。
 *
 * got == 0（FD_INODE 分支）表示 EOF，跟 POSIX read(2) 一致——**返回 0
 * 不是错误**，用户程序据此结束循环。把 EOF 和错误都表示成 -1 的接口用
 * 起来会很难受，因为调用者无法区分"读完了"和"出问题了"。
 *
 ...
 * 本节开头的模块注释）。但请留意那段注释里强调过的那句话：read() 比
...
/* TODO 8：实现 sys_close(fd) -> 0 成功 / -1 失败。
 *
 * 步骤：
 *   1. proc_current()，为 NULL 就 panic。
 *   2. fd_lookup()，为 NULL 返回 -1（关一个没打开的 fd）。
 *   3. **f->type == FD_PIPE：先 pipe_close(f->pipe, f->writable)，再
 *      fd_clear(f)。** 其他类型直接 fd_clear(f)。
 *   4. return 0。
 *
 * 第 2 步那条路径是被真的测到的：user 程序故意做一次重复 close，
 * "关两次第二次要失败"说明槽位真的被清掉了，而不是 close 什么都没做。
 * 设计测试时找这种"能区分两种实现"的观察点，比多测几个正常用例有用
 * 得多。
 *
 * 第 3 步的顺序不能反：管道要先减引用计数、再清槽位，顺序反了就拿不到
 * pipe 指针了。这一行是 Lab9 相对 Lab8 在 close 上唯一的变化，但它承载
 * 了整条管道生命周期链：sh 的六次 close 里每一次都走到这里，管道的两个
 * 计数被一次次减下去，最后归零的那一次触发回收。更重要的是中间那些次——
 * `cat /motd.txt | grep lab` 里 cat 退出时写端计数从 2 减到 1 还不算 EOF
 * （sh 手上还有一份），sh 也关掉之后才归零，grep 这才读到 EOF。少任何
 * 一次 close，grep 就永远挂住。
 *
 * 提示：
...
/* TODO 9：实现 sys_pipe(user_fds) -> 0 成功 / -1 失败。
 *
 * 建一个管道，把读端和写端的 fd 写进 user_fds[0]/[1]。一次系统调用要
 * 返回两个值，而返回值只有一个寄存器——POSIX 的解法是让调用者传一个
 * 数组进来，内核往里写。
 *
 ...
 * 步骤：
 *   1. proc_current()，为 NULL 就 panic。
 *   2. pipe_alloc()，NULL 就返回 -1。
 *   3. rfd = fd_alloc(p)；< 0：两端各 pipe_close 一次（撤销 pipe_alloc
 *      对两个计数的初始假定），返回 -1。
 *   4. 填 p->ofile[rfd]：type=FD_PIPE、inum=0、off=0、pipe=pi、
 *      writable=0。**必须先填这一步，再找写端的 fd**——否则 fd_alloc
 *      会两次返回同一个 fd（它只看 type == FD_NONE，而读端还没填进去）。
 *   5. wfd = fd_alloc(p)；< 0：读端已经占了一个槽位，连它一起回滚——
 *      pipe_close(pi, 0)、fd_clear(&p->ofile[rfd])、pipe_close(pi, 1)，
 *      返回 -1。
 *   6. 填 p->ofile[wfd]：type=FD_PIPE、inum=0、off=0、pipe=pi、
 *      writable=1。
 *   7. user_fds[0]=rfd、user_fds[1]=wfd，返回 0。
 *
 * 步骤 3/5 是本 Lab 里少见的"部分成功之后回滚"的路径。写对它的关键是
 * 认清 pipe_alloc() 把两个计数都初始化成了 1（假定调用者马上会把两端
 * 放进 fd 表），所以回滚就是把那两个假定撤销掉。
 *
 * 提示：
...
/* TODO 10：实现 sys_dup(fd) -> 新 fd（最小可用的）/ -1。
 *
 ...
 * proc.h 里 struct file 的注释。sh 只在管道和重定向里用 dup，两处都不
 * 依赖偏移共享，所以这个偏差在本 Lab 观察不到。
 *
 * ── dup 为什么必须返回最小可用 fd ────────────────────────────────
 *
 ...
 *   1. proc_current()，为 NULL 就 panic。
 *   2. fd_lookup()，为 NULL 返回 -1。
 *   3. nfd = fd_alloc(p)；< 0 返回 -1。
 *   4. p->ofile[nfd] = *f（整个表项拷过去）。**f->type == FD_PIPE：
 *      再 pipe_dup(f->pipe, f->writable)。**
 *   5. return nfd。
 *
 * 第 4 步括起来那句最容易漏：结构体赋值把 pipe 指针也拷过去了，看着
 * "已经对了"——但计数没加。漏掉 pipe_dup 的症状：新 fd 一 close 就把
 * 计数减到比实际引用数还少，另一个仍在使用的 fd 突然看到 EOF 或者 -1。
 * 这是本 Lab 里最容易犯、也最难查的一类 bug，因为出错的地方（另一个 fd
 * 上的 read 提前返回 0）离原因（这里少加了一次）很远。
 *
 * 提示：
                          ...
                          struct context *gpr_snapshot)
{
     ...
     *
     * SYS_READ 用到**三个**参数，这也是这个函数的签名从 Lab8 起就带着
     * a2 的原因（见上面注释）。如果你忘了改 trap_entry.S 的寄存器搬移
     * （那边的 TODO），症状是 a2 里是垃圾值，sys_read 会向 fs_read 传
     * 一个巨大的 len——而 fs_read 会按 inode.size 截断，所以**你可能
     * 看不到崩溃，只是读到的字节数不对**。
     */
    default:
