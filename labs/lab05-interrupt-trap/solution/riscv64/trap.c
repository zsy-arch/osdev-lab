...
/* riscv-privileged 规范 "Machine Cause Register (mcause)" 表格里，
 * S-mode 的中断 code：1=Supervisor Software Interrupt，5=Supervisor
 * Timer Interrupt，9=Supervisor External Interrupt。本 Lab 只有定时器
 * 这一个中断源被使能（sie 只设了 STIE 那一位，见下面 timer_enable()），
 * 其它两种即使 scause 编码上存在，硬件也不会实际产生——但 if 链仍然
 * 显式区分"识别但不是本 Lab 处理的中断"和"完全没见过的 code"两种
 * panic 消息，跟 Lab4 处理未知异常 code 的方式保持同一个风格。 */
...
/* time 是 riscv 定义的非特权级只读 CSR（riscv-privileged 规范
 * "Counters"一节），映射自 CLINT 的 mtime——即使 mtime 本身是 M-mode
 * 的 MMIO 寄存器，S-mode/U-mode 都能通过这个 CSR 直接读到当前时间，
 * 不需要走 SBI（读时间不涉及"编程未来触发一次中断"这种需要 M-mode
 * 代劳的操作，纯读不算特权操作）。 */
...
static void timer_interrupt_handler(void)
{
    ...
    /* riscv64 版 PIT 的关键差异：硬件不会像 x86_64 8254 mode 2 那样
     * 自动周期性重触发，每次处理完这一次中断，必须显式请求"下一次"，
     * 否则这就是最后一次时钟中断——时间基准用 read_time() 现在的值
     * 而不是"上一次预约的值+间隔"，避免处理这次中断本身耗费的时间
     * 被计入下一个间隔，产生累积漂移（教学场景下这个漂移量微不足道，
     * 但用"现在的时间"作为基准是更正确的写法，不需要额外代码就能
     * 避免这个问题，没有理由不这样做）。 */
    uint64_t interval = 10000000ull / TIMER_HZ; /* QEMU virt 平台 mtime
        频率固定 10MHz（QEMU "riscv,virt" 平台的 CLINT 实现约定，见
        QEMU 源码 hw/intc/riscv_aclint.c 和多篇 riscv-virt 文档），
        10000000/100 = 每 tick 间隔 100000 个 mtime 计数。 */
...
void trap_init(void)
{
    /* Direct 模式，跟 Lab4 一样——见 Lab4 版本这里的完整注释：Vectored
     * 模式在本课程只有"page fault + 一个中断源"的规模下不划算，多一种
     * 分发机制的教学收益不足以证明额外复杂度。ROADMAP 把它列成"挑战
     * 任务"，不是本 Lab 主线要求。 */
    uintptr_t entry = (uintptr_t)supervisor_trap_entry;
    __asm__ volatile("csrw stvec, %0" : : "r"(entry));
}

/* 使能监督态时钟中断这条路径上的两个开关（riscv-privileged 规范
 * "Supervisor Interrupt Registers (sip and sie)"、"sstatus"两节）：
