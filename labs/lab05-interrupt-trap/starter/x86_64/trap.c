...
/* 已经写好，不是 TODO：GDT 的内容/加载方式跟 Lab4 完全一样，Lab5 没有
 * 引入任何新知识点——这里不重复解释，见 Lab4 的 trap.c。 */
...
/* TODO 1：确定 IDT_ENTRIES 该写多少。
 *
 * 提示：CPU 保留的异常向量是 0-31（本 Lab 只用到 14 号 #PF），PIC
 * 重映射后 IRQ0-15 落在 32-47（见 pit.c pic_remap() 的映射），本 Lab
 * 只用 IRQ0=32 这一个。数组大小必须能容纳"最大用到的下标+1"——如果
 * 照抄 Lab4 写成 32，idt[32] 会越界写到数组之后紧跟着声明的静态变量
 * （比如下面的 idtp），这不会在编译期报错，运行时也不会立刻崩溃，而是
 * 表现为"第一次定时器中断处理完 iretq 返回后 CPU 状态不对，级联出
 * #GP/#DF"，很容易被误判成 PIC/PIT 硬件配置问题——排查时留意这一点。
 *
...
/* TODO 2：确定 IDT_VEC_TIMER 该写多少。
 *
 * 提示：PIC 重映射后 IRQ0（PIT 定时器）落在的向量号——见 pit.c
 * pic_remap() 里"IRQ0-7 -> 0x20-0x27"的映射，IRQ0 对应 0x20 = 32。
 *
...
/* 已经写好，不是 TODO：读 CR2、page_fault_handler 的内容跟 Lab4 完全
 * 一样，Lab5 没有改动这部分逻辑。 */
...
/* 已经写好，不是 TODO：idt_set_entry() 的实现跟 Lab4 完全一样。 */
...
/* TODO 3：实现 idt_init() —— 在 Lab4 只填 #PF 一项的基础上，多填一项
 * 定时器向量，然后跟 Lab4 一样 lidt。
 *
 * 提示：
 * void idt_init(void)
 * {
