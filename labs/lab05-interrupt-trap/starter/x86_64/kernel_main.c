...
void kernel_boot(uint32_t mb2_info_addr)
{
    ...
    /* TODO 1：把中断真正打开——idt_init() 现在填了两项：#PF（保留
     * Lab4 的能力，虽然本 Lab 不再故意触发它）和定时器（vector 32）。
     * pit_init() 负责重映射 PIC、设置 8254 的 reload value、取消屏蔽
     * IRQ0——但 PIC/PIT 本身不产生"CPU 会响应"的效果，还差最后一步：
     * sti（Set Interrupt Flag，RFLAGS 的 IF 位）。CPU 从复位/boot.S
     * 开始 IF 就是清零状态（这是 x86 架构规定的复位默认值，不是本课程
     * 哪一步意外清掉的），IF=0 时 CPU 会无条件忽略所有可屏蔽外部中断
     * （#PF 这类异常不受 IF 影响，这也是为什么 Lab4 全程没开中断、#PF
     * 依然能正常递送）——sti 必须是这一串初始化里的最后一步，提前
     * 执行会让"IDT 还没填定时器那项/PIC 还没重映射"时就冒出中断，落进
     * 空 IDT entry 触发 #GP 级联。
     *
     * 提示：
     * idt_init();
     * pit_init();
    ...
    /* TODO 2：把主循环从"一次性触发异常然后 panic"（Lab4 的做法）
     * 改成持续运行、周期性汇报的循环。
     *
