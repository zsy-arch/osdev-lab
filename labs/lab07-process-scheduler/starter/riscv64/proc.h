 ...
 *
 * 已经写好，不是 TODO：跟 x86_64 版本（../x86_64/proc.h）同一个理由——
 * 这里的字段顺序是跟 swtch.S 的 sd/ld 顺序、trap_entry.S 的 sd 顺序手工
 * 对齐的硬性契约，不是可以自由选择的开放设计题，你在 swtch.S/trap_entry.S
 * 里实现对应的保存/恢复逻辑时，应该把这里的字段顺序当作已知条件核对，
 * 不需要也不应该重新设计布局。 */
#ifndef OSDEV_PROC_H
#define OSDEV_PROC_H
