...
void kernel_boot(uint64_t hartid, uint64_t dtb_paddr)
{
    ...
    /* TODO 1：调用 proc_init()，然后 proc_alloc() 创建初始进程，
     * proc_alloc() 失败要 panic，最后调用 scheduler()（noreturn，
     * 这个函数从此不会再返回到这里）。
     *
     * proc_init() 清空进程表。proc_alloc() 分配一个全新的 struct
     * proc（内核栈、独立页表、map_user_prog() 映射好用户程序+用户
     * 栈），这是本 Lab 教学范围内唯一一次手工创建进程——后续所有
     * 进程都通过用户态 fork() 系统调用产生。riscv64 这边没有 x86_64
     * 版本的 tss_init() 等价步骤（trap.c 顶部模块注释已经说明：
     * per-process 内核栈顶完全通过 trap_kernel_sp_top 这个全局变量
     * 表达，不需要额外初始化硬件结构），proc_init()/proc_alloc() 之前
     * 不需要补任何东西。
     *
     * 提示：
     * proc_init();
