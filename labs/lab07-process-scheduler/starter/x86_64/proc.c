...
/* TODO 1：实现 proc_init/proc_current。
 *
 * 为什么：proc_init() 把整张进程表清成 PROC_UNUSED（内核启动时调用
 * 一次，kernel_main.c 里在第一次 proc_alloc() 之前),g_current 初始化
 * 成 NULL（此刻还没有任何进程在跑,scheduler() 还没开始)。proc_
 * current() 是给别的文件（目前只有 trap.c)用的只读访问器,避免让
 * g_current 这个 static 变量直接 extern 出去。
 *
 * 提示：
 * void proc_init(void)
 * {
...
/* TODO 2：实现 map_user_prog。
 *
 * 为什么：把内嵌的 user_prog.bin 拷贝进一个新分配的物理页、映射到
 * pagetable 里的 USER_PROG_VADDR，再分配+映射一页用户栈——proc_alloc()
 * （创建全新进程）和 sys_exec()（重置已有进程的地址空间）共用这段
 * 逻辑，是 kernel_main.c（Lab6 版本）里同一段代码的搬移，行为完全
 * 不变，只是从"内联在 kernel_boot() 里、只会跑一次"变成"一个可以被
 * 多次调用的函数"。
 *
 ...
 * 策略）。 */
/* TODO 3：实现 proc_alloc_skeleton。
 *
 * 为什么：在 proc_table 里找一个 PROC_UNUSED 的槽位，把它变成一个
 * "页表/内核栈/trapframe/context 都已经就位，但用户地址空间还完全
 * 没有映射任何东西"的半成品进程——proc_alloc()（全新进程，接下来要
 * map_user_prog())和 sys_fork()（子进程，接下来要逐页拷贝父进程的
 * 映射内容）共用这段骨架，区别只在"用户地址空间里具体放什么"，这
 * 一步完全相同：
 *   - 独立页表（内核范围已经从主内核页表复制过来，见 pagetable_copy_
 *     kernel_range() 的调用）；
 *   - 独立内核栈（每个进程的 trap 现场必须落在自己的栈上，不能共享，
 ...
 * trap_return 执行时 %rsp 停在 context 6 个 callee-saved 寄存器被
 * pop 完之后的位置，正好落在 trapframe 的起始地址——两者紧邻，是
 * 这里手工摆出来的。p->tf/p->context 存的是*内核虚拟地址*（kstack_
 * phys+KERNEL_VIRT_BASE 之后再算的偏移），不是物理地址——涉及*从
 * C 侧解引用*（这里的 memset/写字段）必须先加上偏移，否则复用
 * pagetable_activate() 之后物理地址不再是合法可解引用地址这个坑。
 *
 * 找不到空槽位（4 个都在用）返回 NULL，调用者负责处理（本 Lab 教学
 * 范围内直接返回错误码，不做等待/抢占已有进程这类更复杂的资源回收
 * 策略）。
...
/* TODO 4：实现 proc_alloc。
 *
 * 为什么：创建一个全新进程：骨架 + 内嵌 user_prog.bin 的一份全新
 * 拷贝，作为本 Lab 唯一的"从无到有"的进程创建入口（kernel_main.c
 * 用它创建初始进程；sys_fork() 不用它，因为子进程的用户地址空间
 * 内容来自父进程，不是内嵌镜像的新拷贝，见 sys_fork() 自己的实现）。
 * cs/ss/rflags 三个常量值是"以 ring3 身份第一次进入用户态"这件事
 * 本身固定的取值，不依赖具体是哪个进程。
 *
 * 提示：
...
/* TODO 5：实现 scheduler。
 *
 * 为什么：本 Lab 唯一的"内核主循环"，noreturn——kernel_main.c 里
 * proc_init() + 创建初始进程之后，最后一步就是调用它，此后控制权
 ...
 * for 循环找下一个。回到这里时 g_current 未必还是 p（sys_exit_proc()
 * 会把一个进程标成 ZOMBIE 之后直接切回调度器),这里统一用 p 判断，
 * 不用 g_current，避免依赖"g_current 在 swtch() 返回之后还没被其它
 * 路径改写"这个脆弱的假设。
 *
 * 提示：
 * void scheduler(void)
...
/* TODO 6：实现 yield。
 *
 * 为什么：一个进程主动/被动让出 CPU 的唯一入口。目前只有一个调用点
...
/* TODO 7：实现 sys_exit_proc。
 *
 * 为什么：SYS_EXIT 系统调用最终落到的地方（trap.c syscall_dispatch
 * 会把 SYS_EXIT 路由到这里，取代 Lab6 那个只打印退出码、不真正结束
 * 进程的占位版本）。
 *
 ...
 * 自己的执行流已经彻底停止之后才能做。
 *
 * swtch() 之后紧跟一个 panic()：ZOMBIE 状态的进程永远不会被
 * scheduler() 的 for 循环重新选中（它只挑 PROC_RUNNABLE),所以这次
 * swtch() 调用理论上不应该再返回——如果真的返回了,说明调度器逻辑
 * 本身出了错,应该立刻可见地崩溃,而不是静默地继续执行一段"不应该
 * 存在"的代码路径。
 *
 * 提示：
...
/* TODO 8：实现 fork_copy_page。
 *
 * 为什么：给子进程分配一页物理内存、把父进程在 parent_vaddr 处那一页
 * 的内容整页拷过去、按 flags 映射到子进程页表的同一个虚拟地址——
 * "同一个虚拟地址"是这个简化 fork 能工作的关键：子进程的 trapframe
 * （下面 sys_fork() 里整份拷贝自父进程）里 rip/rsp 存的还是父进程的
 * 虚拟地址值，如果子进程页表里把这些内容映射到不同的虚拟地址，子
 * 进程恢复执行时会读到完全无关的内存。
 *
...
/* TODO 9：实现 sys_fork。
 *
 * 为什么：user_rip_slot/user_rsp_slot 是"父进程这次 SYSCALL 返回后
...
/* TODO 11：实现 sys_wait。
 *
 * 为什么：在 proc_table 里找一个"调用者的 ZOMBIE 子进程"——本 Lab
