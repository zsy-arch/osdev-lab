...
/* Lab7 新增：yield() 是"进程被定时器抢占"这件事真正发生的地方——
 * proc.c 顶部注释、scheduler()/yield() 自己的注释都已经说明了整条
 * 链路的设计，这里只关心"什么时候该调它"这一件事。
 *
 * 必须先 pic_send_eoi() 再 yield()，不能反过来：yield() 内部会
 * swtch() 换栈换执行流，一走就可能是"很久以后"才回到这个函数（等
 * 调度器再一次选中这个进程），如果 EOI 还没发，PIC 会一直以为"上一个
 * 中断没处理完"，卡死所有后续中断（包括让被抢占的进程真正恢复运行
 * 所需要的下一次定时器 tick——一个自己把自己锁死的死锁）。先发 EOI
 * 让 PIC 立刻恢复正常，再决定要不要 yield，两件事在时间上完全独立。
 *
 * proc_current() 为 NULL 时（调度器自己的 for 循环里，没有任何进程在
 * RUNNING，比如所有槠位都是 UNUSED/ZOMBIE 的极端情况，或者调度器
 * 正处在"两次 swtch() 之间"的间隙）不能调 yield()——yield() 内部
 * 直接假设 g_current 非空，见 proc.c 里的实现，这里的判断是这个假设
 * 成立的前提，不是防御性重复检查。 */
