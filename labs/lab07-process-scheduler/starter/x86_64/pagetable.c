...
uintptr_t pagetable_unmap(uintptr_t root, uintptr_t vaddr)
{
     ...
     *
     * 两个决策点：
     * 一是 walk() 的 alloc_missing 传 false——unmap 一个本来就没映射的
     * 地址不应该顺手把中间层节点建出来（那会把"查不到"变成"创建了一堆
     * 空节点再告诉你查不到"），查不到直接返回 0 表示"这里本来就没东西"。
     * 二是清零整项而不是只清 PTE_P 位：残留的地址位和其它标志位如果留在
     * 那里，下一次读这一项的代码很容易把它误当成"看起来还有效"的数据，
     * 整项归零是唯一不留歧义的写法。
     *
     * 返回值取的是清零*之前*那个 entry 里的物理地址部分，所以必须先把
     * entry 读进局部变量，再清零，顺序反了就什么都取不到了。
     *
     * 提示：
     * uint64_t *pt = walk(root, vaddr, false);
     * if (pt == NULL) {
...
/* 已经写好，不是 TODO：一行赋值，上面那段注释已经把"为什么用文件内静态
 * 变量而不是多加一个参数"这个设计说完了，写一遍不会额外学到东西。 */
...
{
    /* TODO 2：把内核根页表的 PML4[511] 这一项复制到 dst_root 的同一个
     * 下标上。
     *
     * 为什么：这是 Lab7 让"每个进程一份页表"真的能跑起来的前提。进程在
     * 用户态执行时 CR3 指向它自己那份表，一旦发生中断/syscall，CPU 会
     * 立刻开始执行内核代码——而此时 CR3 还没换。如果这份进程页表里没有
     * 内核那段映射，取指的第一条内核指令就是缺页，而缺页处理程序自己也
     * 在那段没映射的内核范围里，直接三重错误。所以 proc_alloc() 每建一份
     * 新页表，都必须先把内核范围填好。
     *
     * 只复制一项就够了，理由见上面那段注释（PML4[511] 覆盖 512GiB，本
     * 课程内核镜像和空闲池全在里面）；复制的是*项*本身、共享下级节点，
     * 不是递归深拷贝，动机同一段注释也写了。
     *
     * 两边都要用 table_ptr() 把物理地址转成能解引用的虚拟地址——本文件
     * 里所有页表节点的访问都走这个转换，g_kernel_root 和 dst_root 存的
     * 都是物理地址。
     *
     * 提示：
