...
/* sys_write/sys_exit：跟 x86_64 版本（trap.c 的同名函数)语义完全
 * 一致,包括"用户合法地址在当前页表下对内核也是合法地址,本 Lab 用户
 * 页表和内核页表是同一份,不做地址校验"这个简化——见 x86_64 版本
 * sys_write 上方的完整注释,riscv64 侧共用同一个理由,不重复展开。 */
 ...
 * 的 128 字节布局，sd 顺序 ra/t0-t6/a0-a7)里取出 a7/a0/a1,对应
