/* Lab8：mkfs——把若干个宿主文件打包成一个本课程格式的文件系统镜像。
 *
 *   用法: mkfs <输出镜像> [要放进去的文件...]
 *   例子: mkfs fs.img fsroot/hello.txt fsroot/motd.txt
 *
 ...
 * 里把写路径实现出来才能验证读路径。
 *
 * 为什么参数是"宿主文件列表"而不是把内容硬编码在这个 .c 里：Lab9 要做
 * ELF 加载器和 shell，需要把编译好的用户程序二进制放进镜像里（xv6 的
 * mkfs 就是这么用的：mkfs fs.img README user/_cat user/_echo ...）。
 * 如果这里把文件内容写死在源码里，Lab9 就得把 mkfs 重写一遍。现在这样，
 * Lab9 只需要往命令行后面多加几个文件名。
 *
...
/* 整个镜像先在内存里拼好，最后一次性写盘。32KiB 的镜像完全放得下，
 * 这样做的好处是可以随意回头修改已经"写"过的块（比如最后才知道总共用了
 * 多少块、需要回填位图），不必操心文件偏移的 seek。真实 mkfs 处理几百 GB
 * 的磁盘时当然不能这样，必须流式写入 + 精确 seek。 */
...
/* 从路径里取出文件名部分：fsroot/hello.txt -> hello.txt。
 *
 * 镜像里存的是文件名，不是宿主路径——镜像里的根目录是扁平的（本 Lab 不
 * 支持子目录，见 README 的简化清单），把宿主的目录结构带进去没有意义,
 * 而且 "fsroot/hello.txt" 有 16 个字符，嵌套深一点就会撞上 DIRSIZ 上限。 */
...
static void add_host_file(uint32_t root_inum, const char *path)
{
    ...
    printf("  inode %2u  %-12s %5u 字节  块", inum, name, total);
    for (uint32_t i = 0; i < NDIRECT && di->addrs[i] != 0; i++) {
        printf(" %u", di->addrs[i]);
    }
    printf("\n");
}
