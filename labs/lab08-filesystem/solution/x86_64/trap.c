...
/* fd -> struct file* 的解析。所有三个系统调用共用，把"fd 合法性检查"
 * 收在一处，而不是在每个调用里各写一遍 if。 */
...
int64_t sys_close(int fd)
{
    ...
    struct file *f = fd_lookup(p, fd);
    if (f == NULL) {
        /* 关一个没打开的 fd：返回 -1。用户程序 user_prog.S 故意做一次
         * 重复 close 来验证这条路径——"关两次第二次要失败"说明 used
         * 标志真的被清掉了，而不是 close 什么都没做。 */
        return -1;
    }

    /* 清空整个槽位，不只是 used=0：把 inum/off 也归零，这样"用了一个
     * 已关闭的 fd"在调试时更容易发现（残留的 inum 会让错误看起来像
     * 正常工作）。本 Lab 的 fd 表是进程私有的、没有引用计数，所以
     * close 就只是释放一个槽位，没有"最后一个引用者才真正关闭文件"
     * 这件事——那需要全局 file 表，见 proc.h 里 struct file 的注释。 */
    f->used = 0;
    f->inum = 0;
    f->off = 0;
    return 0;
