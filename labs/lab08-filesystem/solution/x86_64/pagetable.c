 ...
 *   bit 63 NX  1=不可执行。**必须先置位 EFER.NXE 这一位才是合法的**
 *              （boot.S 在开分页之前置,那里有完整说明)。NXE=0 时
 *              bit 63 是一个*保留位*,置上它不是"被忽略",而是让这一项
 *              变成非法页表项——访问该页会直接 #PF,错误码带 RSVD
 *              (bit 3)。这是 Lab4 起潜伏到 Lab8 才发作的一个真实 bug,
 *              boot.S 里 EFER 那段记录了完整过程。
 *
