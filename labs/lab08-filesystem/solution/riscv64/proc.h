 ...
 *
 * 这两段定义跟 x86_64 那份逐字相同，因为"文件描述符是什么"完全不涉及
 ...
 * 返回值约定跟 POSIX 一致：成功返回非负数，失败返回负数。这三个原型跟
 * x86_64 那份逐字相同——它们不碰任何架构专属的现场（sepc/trapframe），
 * 只读写 struct proc 里的 ofile[] 和调用 fs.c，所以两边的实现也是
 * 逐字相同的。对照 sys_fork/sys_exec 两边签名都不一样（见上面那段长
 * 注释），这个差别本身就说明了"哪些内核功能真的跟架构有关"。 */
