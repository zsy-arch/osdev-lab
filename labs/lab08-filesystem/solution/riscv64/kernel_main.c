...
void kernel_boot(uint64_t hartid, uint64_t dtb_paddr)
{
     ...
     * 设备树里写的地址"对不上。
     *
     * 一个不需要额外操作的巧合值得指出：(0x10001000 >> 30) & 0x1ff == 0，
     * 跟 UART 的 0x10000000 落在同一个顶级页表项里（pagetable.c 的
     * KERNEL_UART_ROOT_INDEX 就是 0）。pagetable_copy_kernel_range()
     * 只拷贝两个顶级项（内核镜像那个 + UART 那个），所以这 8 页自动
     * 出现在每个进程的页表里，不需要给 copy_kernel_range 增加第三个项。
     * 如果 virtio 的地址落在别的 1GiB 区间，这里就必须同时改
     * pagetable.c——这正是"内核映射靠顶级项共享传播"这个机制会隐式
     * 依赖地址布局的地方，换机型（比如 sifive_u）时要重新检查。 */
    ...
    /* Lab8：块设备 + 文件系统初始化。位置的约束跟 x86_64 版本完全一样
