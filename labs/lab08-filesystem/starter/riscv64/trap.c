...
/* TODO 1 ~ TODO 4：实现 fd_lookup / sys_open / sys_read / sys_close。
 *
 * 这四个的**答案跟 x86_64 版本逐字相同**——包括注释。完整的分步说明、
 * 每一步的理由、以及几个容易写错的地方（负 fd 下标、"文件不存在该返回
 * -1 还是 panic"、f->off += got 这一行的意义、close 要清整个槽位而不只是
 * used=0）都写在 ../x86_64/trap.c 的同一节里，这里不重复。
 *
 * 建议的做法：先在 x86_64 下写完并跑过测试，然后把这四个函数整段拷过来。
 * 拷完之后回头想一下为什么它们能这样拷——本节开头那段注释给了判据：
 * 它们只碰 struct proc 里的 ofile[] 和 fs.c，都是我们自己定义的东西，
 * 不碰任何 CSR/段寄存器/页表位布局。sys_fork/sys_exec 两边签名都不一样，
 * 因为那两个操作的是"陷入现场"，是架构定义的东西。
 *
 * 唯一需要自己写的 riscv 相关部分是下面 syscall_dispatch 里的三个 case
 * （TODO 5），以及 kernel_main.c 里的 virtio MMIO 映射。
 *
 * 提示（四个函数的骨架，细节见 x86_64 版本）：
...
int64_t syscall_dispatch(uint64_t num, uint64_t a0, uint64_t a1, uint64_t a2)
{
    ...
    /* TODO 5：加上 SYS_OPEN / SYS_READ / SYS_CLOSE 三个 case，跟 x86_64
     * 版本完全一样：
     *   SYS_OPEN  -> sys_open((const char *)a0);
     *   SYS_READ  -> sys_read((int)a0, (void *)a1, a2);
     *   SYS_CLOSE -> sys_close((int)a0);
     *
     * 注意上面那段注释说的事：a2 这个第三个参数在 riscv64 侧**不需要改
     * 任何汇编**，因为 trap_entry.S 的栈帧从 Lab4 起就一直在保存 a0-a7
     * 全部八个寄存器。x86_64 那边要动 trap_entry.S。做完两边之后回头对比
     * 一下这个差异的来源——它是当初保存策略的取舍带来的，不是架构优劣。
     */
    default:
